...
const scriptOf = (body: unknown): string => {
  const command = (body as { startCommand: string }).startCommand;
  const encoded = /^\/bin\/sh -c printf\$\{IFS\}%s\$\{IFS\}([A-Za-z0-9+/=]+)\|base64\$\{IFS\}-d\|\/bin\/sh$/.exec(
    command,
  )?.[1];
  assert.ok(encoded, `unexpected Render job command: ${command}`);
  return Buffer.from(encoded, "base64").toString();
};
const scriptOperation = (body: unknown): "enable" | "disable" =>
  scriptOf(body).includes("admin user disable") ? "disable" : "enable";
...
`,
  ...
  const f = fixture(async ({ method, body }) => {
    ...
    if (scriptOperation(body) === "disable") {
      const [shell, ...args] = (body as { startCommand: string }).startCommand.split(" ");
      status = await promisify(execFile)(shell!, args, {
        env: {
          ...process.env,
...
test("Rate-limited Render storage submissions keep their repair intent and resubmit once", async () => {
  let rejected = true;
  let jobs = 0;
  let listed = 0;
  ...
  const id = randomUUID();
  await assert.rejects(f.storage.ensure(id), /http 429/);
  assert.equal((await f.store.get(id))!.operation, "enable");
  assert.ok((await f.store.get(id))!.jobRequest);
  rejected = false;
  await f.storage.ensure(id);
...
for (const status of [400, 401, 403, 404, 409, 410, 412, 422])
  test(`Rejected Render storage submissions (${status}) drop repair intent and allow a new job`, async () => {
    let rejected = true;
    let jobs = 0;
    const f = fixture(({ method, path }) => {
      if (method === "POST") {
