...
A plan for hosting qm on Render: the control plane first, then Render Sandboxes as the
agent computer, then Render as a first-class `qm` deployment target. Written against the
qm tree at `234022f` and the Render public API as reflected in `render-oss/sdk` and
`render-oss/cli` (September 2026). Everything here that touches core qm is upstream work;
this fork keeps only its own layer under `deploy/layers/render/`.
...
| ----- | ------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------- | ---------------------- |
| 0     | qm control plane live on Render from a Blueprint, agent computers on an existing API-driven backend (Sprites, E2B, Modal or Agent37) | nothing new                                         | days                   |
| 1     | `SANDBOX_BACKEND=render` on Render Sandboxes, upstreamed like the Agent37 backend                                                    | Sandboxes early access                              | 1–2 weeks              |
| 2     | `qm init --target render`: Render in the CLI's hosting-provider registry next to Fly and AWS                                         | Phase 0 learnings                                   | 2–3 weeks              |
| 3     | `DEPLOY_PROVIDER=render` so apps the agent publishes run on Render                                                                   | a Render primitive that does not exist yet (see §7) | spike now, build later |
...
enforced egress on Render without running its Envoy proxy at all. §9 lists the other gaps.
...
| egress-proxy                    | `type: pserv` on 48080 — only if sandboxes can reach the private network                                                                                                   | See §5.                                                                                                                                                                                                                                             |
...
| Agent computers                 | Phase 0: Sprites / E2B / Modal / Agent37 with their tokens. Phase 1: Render Sandboxes                                                                                      |                                                                                                                                                                                                                                                     |
...
### Blueprint draft (Phase 0)

This draft belongs at `deploy/layers/render/render.yaml` once Phase 0 starts (it is
...
services:
  - type: pserv
    ...
    envVars:
      ...
      - { key: SANDBOX_BACKEND, value: sprites } # render, once Phase 1 lands
      - { key: SPRITES_TOKEN, sync: false }
...
- `AUTH_CLIENT_SECRET` and `OIDC_CLIENT_SECRET` must carry the same value, and
  ...
  value under two names", so in Phase 0 they are generated locally the way `qm setup`
...
- `PUBLIC_API_URL` (the address sandboxes use to call core for blob staging) is deliberately
  absent. Set it only once there is a route from the sandboxes to core: the private hostname
  if Render Sandboxes join the workspace network, otherwise a public passthrough. Blob
  staging is simply not advertised while it is unset.

## 3. Phase 0 — control plane on Render today

Goal: prove the platform fit end to end without new code, using an existing API-driven
sandbox backend.
...
2. **Provision.** Create the Blueprint from the fork (the dashboard's Blueprint flow reads
   ...
   four resources with `render services create` and `render postgres create`, which are
...
3. **Secrets.** Generate the five signing secrets and the broker material locally, choose
   ...
   Slack-first company), mint the sandbox token for the stopgap backend, and set
   `ADMIN_GRANTS` before the first boot: without it the admin console is permanently
   unreachable (`docs/porter.md`, step 5).
...
5. **Verify** the same way `deployment.md` step 4 does: `/healthz` on all three, sign in
   ...
   to write a UUID to `/root/workspace/qm-computer-proof.txt` and read it back through the
   sandbox provider's own tooling, then invite the Slack bot and get a reply.
6. **Soak with two core instances** once object storage is in place: `numInstances: 2`
   exercises the leader leases, pg-boss singleton jobs and the `GIT_SHA` drain on a real
   rolling deploy. Every existing template pins core to one instance, so this is the first
   time the multi-instance design meets a platform that makes it the default.

Known limitations in this phase: no egress enforcement unless the proxy is reachable from
the chosen sandbox provider; publishing unavailable; sandboxes billed by a third party.

## 4. Phase 1 — Render Sandboxes as the agent computer

This is the piece that makes Render a real home for qm rather than a host for its control
plane. The Agent37 ADR (`adrs/agent37-sandbox-backend.md`) is the template: one backend
file modeled on an existing one, an in-process fake of the vendor API, tests, and no new
CLI target. The shape of the Render backend follows from the API:
...
arrive through the deployment-layer sync, so the snapshot rarely changes.
...
like the E2B and Modal smokes; `docs/sandbox-preservation.md`; `.env.example`. Upstream it
with an ADR in the style of `adrs/agent37-sandbox-backend.md`.

Questions for the Render side that decide details (all answerable internally before the
first line of code): the run duration and output limits behind a connect token; whether
sandboxes can reach the workspace private network (this decides both egress enforcement
and `PUBLIC_API_URL`); what puts a sandbox into `suspended` and whether idle time is
billed; the default and maximum snapshot lifetime (`expiresAt` is set by Render when
omitted, and qm needs scope snapshots to outlive any default); the default per-group
concurrency limit; the base image's user, home directory and preinstalled toolchain; and
whether names or labels can be attached so an operator can recognize a scope's sandbox in
the dashboard.

## 5. Egress enforcement on Render
...
## 6. Phase 2 — `qm init --target render`
...
## 7. Phase 3 — `DEPLOY_PROVIDER=render` for published apps
...
Recommendation: do not block Phases 0–2 on this. Spike the "promote to a Render service"
flow now; ship the default provider when either sandboxes expose ports or archive builds
exist.

## 8. Operating qm on Render
...
## 9. What this surfaces for Render's roadmap
...
- **Lifetime cap mid-session.** Rotation before `timeoutSeconds` must be invisible to the
  user; qm has the pattern, but it has to be tested against Render's actual timings.
- **Multi-instance core is untested in production.** The design is there and the tests
  pass, but every existing template pins one instance. Phase 0's soak is where that debt
  gets paid.
- **Fork discipline.** Everything under §4, §6 and §7 is core and goes upstream through the
  `upstream-pr` skill; only `deploy/layers/render/` stays here.

## 11. Suggested order of work

1. Phase 0 live, with the Blueprint and runbook in this layer and a stopgap sandbox
   backend chosen (Sprites needs a Fly token, Agent37 and E2B need only an API key).
2. Answer the §4 questions internally, then build the Render sandbox backend with its fake
   and smoke; open the upstream ADR and PR.
3. Phase 2 target, docs and fixtures; retire the hand-maintained Blueprint from the layer
   once `qm plan` renders it.
