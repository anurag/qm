...
import { createHomeSnapshotOps, HOME_SNAPSHOT_PRUNE, type HomeSnapshotStore } from "./home-snapshot.ts";
...
import { visibleTools, type ComputerStatus, type ExecResult, type Sandbox } from "./sandbox.ts";
import {
...
export function createRenderSandbox(workspace: WorkspaceStore, opts: RenderSandboxOptions): Sandbox {
  ...
  async function checkpoint(scope: string, stored: StoredRenderSandbox): Promise<StoredRenderSandbox> {
    ...
    if (stored.pendingCheckpoint) stored = await finishCheckpoint(scope, stored);
    ...
    try {
      ...
      published = true;
      return await finishCheckpoint(scope, await cleanupRetired(scope, saved));
    } catch (error) {
      if (!published) await cleanupRetired(scope, (await store.get(scope)) ?? pending);
      throw error;
    }
  }
  ...
  async function ensureResident(name: string, onStatus?: (text: string) => void): Promise<{ coldStart: boolean }> {
    ...
    return withLock(name, async () => {
      ...
      if (stored) {
        ...
        if (stored.pendingCheckpoint) {
          stored = await finishCheckpoint(scope, stored).catch(async (error) => {
            if (!(error instanceof RenderSnapshotGoneError)) throw error;
      ...
      try {
        ...
        if (snapshotId) {
          const scrub = await client.runCommand(
            info.id,
            `rm -rf /tmp/agent-creds ${shq(homeDir)}/.cred-state*`,
            30_000,
          );
          if (scrub.exitCode !== 0)
  ...
  return {
    ...
    async provision(layers, options) {
      ...
      if (handle.coldStart && !handle.scratch) {
        await withLock(handle.id, async () => {
          const scope = scopeFor(handle.id);
          const stored = await store.get(scope);
          if (stored && !stored.checkpoint) await checkpoint(scope, stored);
        });
      }
      return handle;
    },
    ...
    ...files,
    ...exports,
    ...createBackendBlobStaging("render", exec, opts),
    ...processes,
    destroyScope,
    ...
    async teardown(handle, options) {
      if (handle.scratch) return base.teardown(handle, options);
      const scope = scopeFor(handle.id);
    ...
    async reapDeepIdle(idleMs) {
      ...
      for (const [scope, record] of await store.entries()) {
        if (record.lastActivityMs > cutoff) continue;
        await withLock(nameFor(scope), async () => {
          let stored = await store.get(scope);
          if (stored) stored = await recoverStored(scope, stored);
          if (!stored || stored.lastActivityMs > cutoff) return;
          const info = await client.get(stored.sandboxId);
          if (info?.status !== "running") return;
          if (await hasLiveProcesses(info.id)) return;
          await checkpoint(scope, stored);
          await client.terminate(info.id);
          reaped++;
