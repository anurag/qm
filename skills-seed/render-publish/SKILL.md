...
Use an S3 client for files. Read `AWS_ENDPOINT_URL_S3`, `AWS_REGION`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `S3_BUCKET`, and `S3_PREFIX` from the environment. Enable path-style requests. Prepend `S3_PREFIX` to every object key and list prefix. The credentials allow access only to that app's prefix. Do not use the root bucket as a list prefix. For AWS SDK JavaScript, set `endpoint`, `region`, `forcePathStyle: true`, and `requestChecksumCalculation: "WHEN_REQUIRED"` on `S3Client`.
