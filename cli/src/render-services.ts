...
export function renderServiceEnv(
  ...
  for (const secret of computedSecrets(config)) {
    const value = secret.managedBy === "render" ? managedValues[secret.name] : values.get(secret.name);
    if (value === undefined || secret.managedBy === "terraform") continue;
  ...
  if (service === "core")
    Object.assign(out, {
      ...
      ...(connections.commit ? { RENDER_DEPLOY_COMMIT: connections.commit, GIT_SHA: connections.commit } : {}),
  ...
  if (service === "core") {
    ...
    Object.assign(out, {
      ...
      AWS_ENDPOINT_URL_S3: connections.storage?.endpoint ?? "",
      AWS_ACCESS_KEY_ID: "qm-storage",
      AWS_SECRET_ACCESS_KEY: connections.storage?.secretKey ?? "",
      AWS_SESSION_TOKEN: "",
    });
  }
  if (service === "portal") {
