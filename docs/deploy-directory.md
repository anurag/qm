...
The root object requires `contract: 1`, `orgId`, `publicUrl`, `target`, and `services` including `core`. On AWS the sandbox substrate is an explicit choice: omitting the `sandbox` block runs named Lambda MicroVM images; declaring one requires `sandbox.backend` — `"sprites"` runs Fly Sprites, `"aws"` states the MicroVM default in the file. Unknown contract majors fail closed. `target` is `docker`, `fly`, `aws`, or `render`.
...
| Requirement                                                                                |                          Docker |                             Fly |                                             AWS |                                                   Render |
| ------------------------------------------------------------------------------------------ | ------------------------------: | ------------------------------: | ----------------------------------------------: | -------------------------------------------------------: |
| Node 24 and `qm` CLI                                                                       |                             yes |                             yes |                                             yes |                                                      yes |
| Docker daemon                                                                              |                             yes |                      build path |                       image transfer/build path |                               no; Render builds from Git |
| Agent-computer image and credentials                                                       |      Fly app for real execution |        Fly app and scoped token | Lambda MicroVM image/version and execution role | Render API key and workspace for native Render Sandboxes |
...
| Postgres                                                                                   | local container or supplied DSN |       Fly Postgres/supplied DSN |                                   Terraform RDS |                                          Render Postgres |
| AWS credentials, ECS/ECR/RDS/ALB/Cloud Map, exact GitHub OIDC trust                        |                              no |                              no |                                             yes |                                                       no |
